...
-- name: DummyQuery :one
SELECT CASE WHEN NOT EXISTS (SELECT 1 FROM whitelistMinters WHERE whitelistMinters.discord_id = $1) THEN (UPDATE whitelistMinters SET discord_id = $1, discord_username = $2, avatar_hash = $3, updated_at = $4
WHERE nonce = (SELECT nonce FROM whitelistMinters WHERE whitelistMinters.discord_id IS NULL LIMIT 1)) END;